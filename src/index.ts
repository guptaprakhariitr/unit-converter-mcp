...
const SERVER_INFO = { name: "unit-converter-mcp", version: "0.1.0" };
...
const LLMS_TXT = `# unit-converter-mcp

> Convert units, currencies, timezones, dates for AI agents. The boring utility every agent needs.

## Tools
- convert_unit(value, from, to) — 10 categories, 60+ units
- list_units() — full unit dictionary
- convert_currency(amount, from, to, date?) — live FX
- convert_timezone(iso, from_tz, to_tz)
...
function renderLanding(env: Env, url: URL): string {
  ...
  const tagline = env.PRODUCT_TAGLINE ?? "MCP server for AI agents: convert units, currencies, timezones, and date arithmetic.";
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>unit-converter-mcp</title>
